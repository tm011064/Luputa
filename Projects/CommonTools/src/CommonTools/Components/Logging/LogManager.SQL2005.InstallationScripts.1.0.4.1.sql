...
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'{ DatabaseOwner }.[{ StoredProcedurePrefix }Exceptions_GetPage]') AND type in (N'P', N'PC'))
	SET @AlterCreate = 'CREATE ';
SET @AlterCreate =  @AlterCreate + CAST(' PROCEDURE [dbo].[proc_ct_Exceptions_GetPage]
(
	@PageIndex			INT, 
    @PageSize			INT,
	@RowCount			INT OUTPUT,
...
BEGIN
	...
	BEGIN
		SET @WhereClause = '' WHERE	ct_Exceptions.AppLocation = '' + @AppLocationFilter + '' AND	ct_Exceptions.HandlingStatus = '' + @HandlingStatus;
	END
	ELSE IF @AppLocationFilter >= 0
	BEGIN
		SET @WhereClause = '' WHERE	ct_Exceptions.AppLocation = '' + @AppLocationFilter;	
	END
	ELSE IF @HandlingStatus >= 0
	BEGIN
		SET @WhereClause = '' WHERE	ct_Exceptions.HandlingStatus = '' + @HandlingStatus;
	END
	ELSE
	BEGIN
		SET @WhereClause = '''';
	...
	SET @Command = ''SELECT  el.[ExceptionID]
				  ...
				  , el.[MachineName]
				  , el.[AuthenticatedUserId]
			FROM (  SELECT	  TOP (('' + CAST(@PageIndex AS NVARCHAR(32)) + '' + 1) * '' + CAST(@PageSize AS NVARCHAR(32)) + '') ROW_NUMBER() OVER (ORDER BY ct_Exceptions.''+@OrderByCommand+'') AS [ROW_NUMBER]
							, [ExceptionID] AS ExceptionID
					FROM		ct_Exceptions '' + @WhereClause + ''
