...
namespace CommonTools.Components.Graphics
{
    ...
    public static class ImageHelper
    {
        ...
        public static Image Resize(Image image, Size size)
        {
            ...
            percentWidth = ((float)size.Width / (float)image.Width);
            percentHeight = ((float)size.Height / (float)image.Height);

            percent = (percentHeight < percentWidth) ? percentHeight
                                                     : percentWidth;

            int destWidth = (int)(image.Width * percent);
            int destHeight = (int)(image.Height * percent);
        ...
        /// <returns></returns>
        public static Image Crop(Image image, int posX, int posY, int width, int height)
        {
            if (posX + width > image.Width)
                width = image.Width - posX;
            if (posY + height > image.Height)
                height = image.Height - posY;
        ...
        public static ImageFormat GetImageFormatFromMimeType(string mimeType)
        {
            switch (mimeType.ToLower())
            {
                ...
                case "windows/metafile": return ImageFormat.Wmf;
            }
            return null;
        }
        ...
        {
            return GetMimeType("image" + GetFileExtension(imageFormat));
        }
