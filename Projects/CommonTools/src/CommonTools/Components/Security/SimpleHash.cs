...
namespace CommonTools.Components.Security
{
    ...
    public static class SimpleHash
    {
                                         ...
                                         byte[] saltBytes)
        {
            ...
            byte[] pTextBytes = Encoding.UTF8.GetBytes(pText);

            // Allocate array, which will hold plain text and salt.
            byte[] pTextWithSaltBytes =
                    new byte[pTextBytes.Length + saltBytes.Length];

            // Copy plain text bytes into resulting array.
            for (int i = 0; i < pTextBytes.Length; i++)
                pTextWithSaltBytes[i] = pTextBytes[i];

            // Append salt bytes to the resulting array.
            for (int i = 0; i < saltBytes.Length; i++)
                pTextWithSaltBytes[pTextBytes.Length + i] = saltBytes[i];
            ...
            byte[] hashBytes = hash.ComputeHash(pTextWithSaltBytes);
            ...
            string pCipherText = Convert.ToBase64String(hashWithSaltBytes);
                                      ...
                                      string pCipherText)
        {
            ...
            byte[] hashWithSaltBytes = Convert.FromBase64String(pCipherText);
            ...
            string expectedHashString =
                        ComputeHash(pText, pHashAlgorithm, saltBytes);

            // If the computed hash matches the specified hash,
            // the plain text value must be correct.
            return (pCipherText == expectedHashString);
        }
    }
