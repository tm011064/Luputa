...
namespace Workmate.Web.Components.Security
{
  ...
  public static class AuthenticationCookieManager
  {
    ...
    internal static HttpCookie GenerateAuthenticationCookie<T>(string username, int authenticationTimeoutInSeconds, int persistentAuthenticationTimeoutInSeconds
        , T userObject, bool? isPersistent, CookieGenerationMode cookieGenerationMode, string domain) where T : IProtobufSerializable
    {
      ...
      switch (cookieGenerationMode)
      {
        ...
        case CookieGenerationMode.CreateNew:
        case CookieGenerationMode.UpdateExisting:
          userData = Convert.ToBase64String(userObject.Serialize());
          break;
      }
    ...
    public static void WriteAuthenticationCookie<T>(string username, int authenticationTimeoutInSeconds, int persistentAuthenticationTimeoutInSeconds
        , T userObject, bool? isPersistent, string domain) where T : IProtobufSerializable
    {
    ...
    /// <param name="isPersistent">The is persistent.</param>
    public static void WriteAuthenticationCookie<T>(string username, int authenticationTimeoutInSeconds, int persistentAuthenticationTimeoutInSeconds
        , T userObject, bool? isPersistent) where T : IProtobufSerializable
    {
    ...
    /// <typeparam name="T"></typeparam>
    /// <returns></returns>
    public static T GetEmbeddedDataFromAuthenticationCookie<T>() where T : IProtobufSerializable
    {
      ...
      if (user != null && user.Identity.IsAuthenticated)
      {
        ...
        T item;
        using (MemoryStream ms = new MemoryStream(Convert.FromBase64String(ticket.UserData)))
